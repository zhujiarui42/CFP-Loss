import numpy as np
import torch
from core.lungseg import seg_utils
import SimpleITK as sitk
from core.lungseg.resunet import UNet
import warnings
import sys
...
def apply(image, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    ...
    if not noHU:
        tvolslices, xnew_box = seg_utils.preprocess(inimg_raw, resolution=[256, 256])
    ...
    torch_ds_val = seg_utils.LungLabelsDS_inf(tvolslices)
    ...
    if volume_postprocessing:
        outmask = seg_utils.postrocessing(timage_res)
    else:
        outmask = timage_res
    ...
    if noHU:
        ...
        outmask = np.asarray(
            [seg_utils.reshape_mask(outmask[i], xnew_box[i], inimg_raw.shape[1:]) for i in range(outmask.shape[0])],
            dtype=np.uint8)
...
def apply_fused(image, basemodel='LTRCLobes', fillmodel='R231', force_cpu=False, batch_size=20,
    ...
    return seg_utils.postrocessing(res_l, spare=[spare_value])
